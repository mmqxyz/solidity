...
void Assembly::importAssemblyItemsFromJSON(Json::Value const& _code, vector<string> const& _sourceList)
{
	...
	for (Json::Value const& jsonItem: _code)
		m_items.emplace_back(createAssemblyItemFromJSON(jsonItem, _sourceList));

	for (auto current = m_items.begin(); current != m_items.end(); ++current)
	{
		// Tags are very similar to `JUMPDEST`s. However, during the
		// assembly json export a `JUMPDEST` is always generated after a `tag`.
		// So we need to ignore exactly these `JUMPDEST`s during the import.
		auto const nextIter = next(current);
		if (
			nextIter != m_items.end() &&
			current->type() == AssemblyItemType::Tag &&
			nextIter->type() == AssemblyItemType::Operation &&
			nextIter->instruction() == Instruction::JUMPDEST
		)
...
AssemblyItem Assembly::createAssemblyItemFromJSON(Json::Value const& _json, std::vector<std::string> const& _sourceList)
{
	...
	solRequire(isOfType<string>(_json["name"]), AssemblyImportException, "Member 'name' not of type string.");
	...
	auto valueMustBeDefined = [&](string const& _name, string const& _value)
	{
		solRequire(
			!_value.empty(),
			AssemblyImportException,
			"Member 'value' was not defined for instruction '" + _name + "', but instruction need a value."
		);
	};

	auto valueMustBeUndefined = [&](string const& _name, string const& _value)
	{
		solRequire(
			_value.empty(),
			AssemblyImportException,
	...
	if (srcIndex >= 0 && srcIndex < static_cast<int>(_sourceList.size()))
		location.sourceName = std::make_shared<std::string>(_sourceList[static_cast<size_t>(srcIndex)]);
	else if (srcIndex < -1 || srcIndex >= static_cast<int>(_sourceList.size()))
	...
	if (c_instructions.count(name))
	{
		...
		if (info.args == 0)
			valueMustBeUndefined(name, value);
		result = item;
	}
	else
	{
...
shared_ptr<Assembly> Assembly::fromJSON(Json::Value const& _json, vector<string> const& _sourceList, int _level)
{
	...
	if (_json.isMember(".data"))
	{
		...
		for (Json::ValueConstIterator dataIter = data.begin(); dataIter != data.end(); dataIter++)
		{
			...
			{
				shared_ptr<Assembly> subassembly(Assembly::fromJSON(code, sourceList, _level + 1));
